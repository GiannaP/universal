...
namespace sw {
  namespace unum {
    ...
    template<size_t nbits>
    bool lessThan_twoscomplement(const bitblock<nbits>& lhs,
                                 const bitblock<nbits>& rhs) {
      // comparison of the sign bit
      unsigned int signBit = nbits - 1;
      if (lhs[signBit] == 0) {
        return rhs[signBit] == 0 ? lhs < rhs : false;
      }
      else {
        return rhs[signBit] == 1 ? lhs > rhs : true;
      }
    }

    /**
      @brief Vestigial alias for '<' operator.
    */
    template<size_t nbits>
...
#         endif
    ...
    /** 
        @brief Rounds off \p n last bits of bitset \p src. Round to nearest resulting in potentially smaller bitset.
  Doesn't return carry bit in case of overflow while rounding up! TODO: Check whether we need carry or we require an extra bit for this case.
    ...
    */
    ...
    bitblock<nbits> convert_to_bitblock(const Type &number) {
      bitblock<nbits> Bits;
    ...
    template<size_t nbits>
    std::string to_binary(const bitblock<nbits> &bits) {
      std::stringstream ss;
    ...
    template<size_t nbits>
    std::string sign_magnitude_to_string(const bitblock<nbits> &bits) {
      std::stringstream ss;
      ss << (bits[nbits - 1] ? "n-" : "p-");
